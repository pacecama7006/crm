                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="description" class="required">Descripción del proyecto</label>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                ...
                                <select name="user_id" id="user_id" class="form-control select2" style="width:100%; ">
                                    ...
                                    @foreach ($users as $id => $user)
                                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $project->user->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $user }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-md-4">
                                <label for="status" class="required">Status del proyecto</label>
                                <select name="status" id="status"
                                    class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}"
                                    style="width: 100%;" required>
                                    <option value="">Seleccione un status</option>
                                    @foreach (App\Models\Project::STATUS as $status)
                                        <option value="{{ $status }}" {{ (old('status') ? old('status') : $project->status ?? '') == $status ? 'selected' : '' }}>
                                            {{ $status }}
                                        </option>
                                    @endforeach
                                </select>
